...
resource "aws_iam_role" "RoleForCognito" {
  name = "example_role"
...
resource "aws_iam_role_policy" "rolePolicy" {
  ...
  role = aws_iam_role.RoleForCognito.name
}
...
resource "aws_iam_role_policy_attachment" "s3_full_access_policy_attachment" {
  ...
  role       = aws_iam_role.RoleForCognito.id
}
...
resource "aws_cognito_identity_pool_roles_attachment" "example" {
  ...
  roles = {
    "authenticated"   = aws_iam_role.RoleForCognito.arn
    "unauthenticated" = aws_iam_role.RoleForCognito.arn
  }
}
